%% ============================================================================
%% radar_lts_19870416.pl  --  Labeled Transition System for RADARPAS
%%                            (1987-04-16 "RADARPAS RadioTx Enhanced",
%%                             banner: "ver 2.2 Revision 4/13/87")
%% ============================================================================
%%
%% Source program:  radar/19870416 RADARPAS RadioTx Enhanced/RADAR.PAS
%%                  (+ RADIO.PAS, CUSTOM.PAS)
%% Companion files: radar/radar_lts_1985.pl
%%                  radar/radar_lts_19870412.pl
%%
%% Four days after the 19870412 snapshot.  The state shape, action vocabulary,
%% and most guards/updates are identical to radar_lts_19870412.pl.  This file
%% records the small set of behavioural changes visible at the LTS level:
%%
%%   1. HangUp's hardware step is commented out (RADAR.PAS line 147), so
%%      dropping the modem call no longer drops carrier.  Callers still
%%      assign Mode := Modem, so the `hang_up` LTS transition is unchanged
%%      in *application state*; only the underlying physical line stays up.
%%      This is reflected in a header comment, not in the transition relation.
%%
%%   2. SendCom now displays "TIMED OUT" when its DelTime expires
%%      (RADAR.PAS line 175).  UI only.
%%
%%   3. SendCom('X', ...) timeout in InterLoop drops from 6000 ms to 1000 ms
%%      (RADAR.PAS lines 1051-1053).  Timing only.
%%
%%   4. ModemLoop F2 path replaces "CallStation; if Mode=Interactive then
%%      InterLoop;" with "Mode:=Interactive; InterLoop;"  (RADAR.PAS lines
%%      1118-1119).  The user can now enter Interactive mode without
%%      attempting a dial -- a development convenience.  This LTS adds a
%%      new action `enter_interactive_no_dial` for that path.
%%
%%   5. InitRS232 no longer dual-initialises the host port; the modem-init
%%      sequence sends 'ATS10=255' instead.  Setup only -- no LTS impact.
%%
%% Naming, predicate signatures, and structural conventions match
%% radar_lts_1985.pl and radar_lts_19870412.pl.
%%
%% ----------------------------------------------------------------------------

:- module(radar_lts_19870416, [
        initial_state/1,
        guard/2,
        step/3,
        trans/3,
...
%% --- New in 19870416 ------------------------------------------------------
action(enter_interactive_no_dial).  % ModemLoop F2 path: Mode:=Interactive
                                    % without invoking CallStation
                                    % (RADAR.PAS lines 1118-1119).
...
%% New in 19870416: F2 in ModemLoop bypasses CallStation entirely.  No phone
%% number is required and no dial outcome is consulted.
guard(enter_interactive_no_dial, S) :-
    st_mode(S, modem).
...
step(enter_interactive_no_dial, S0, S1) :-
    set_mode(S0, interactive, S1).        % F2 dev bypass; no carrier required
...
%%   true.
%%
%%   %% New in 19870416: F2 enters Interactive without dialling, even with
%%   %% no station loaded.
%%   ?- initial_state(S0),
%%      trans(enter_interactive_no_dial, S0, S1),
%%      st_mode(S1, M).
%%   M = interactive.
%% ----------------------------------------------------------------------------
