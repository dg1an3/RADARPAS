%% ============================================================================
%% radar_lts_19880114.pl  --  Labeled Transition System for RADARPAS
%%                            (1988-01-14 "RADAR.PAS" standalone snapshot,
%%                             banner: "ver 2.1  Revision 1/14/88
%%                             Copyright (C) 1987 D. G. Lane")
%% ============================================================================
%%
%% Source program:  radar/19880114 RADAR.PAS  (single self-contained file,
%%                  with MISC.RDR / GRAPH.RDR / SCREEN.RDR includes inlined)
...
%%
%% A Pascal-mainline continuation, eight months after 19870416 and two
%% months after the parallel Modula-2 port.  Behaviourally very close to
%% 19870412 (v2.0); the major LTS-level differences are:
%%
%%   1. F4 RadioTx is removed from ModemLoop (RADAR.PAS lines 1835-1862).
%%      The `radio_tx` action is therefore dropped from the vocabulary.
%%
%%   2. CallStation is restored to its proper dial path (the 19870416
%%      "F2 -> Mode:=Interactive directly" dev bypass is gone).  The
%%      `enter_interactive_no_dial` action from radar_lts_19870416.pl is
%%      therefore dropped here.
%%
%%   3. NEW: a no-phone fallback in CallStation.  When PhoneNum='' but
%%      StationName is set, Mode:=Interactive without dialling
%%      (RADAR.PAS lines 1256-1259).  Modelled as `call_no_phone`.
%%
%%   4. HangUp's body is still commented out (RADAR.PAS line 854); like
%%      19870416, the action transitions Mode := Modem at the LTS level
%%      while the underlying carrier stays up.
%%
%%   5. SetDir gains (Mask, Attr) parameters; ResetBuf added; SendCom
%%      timeouts shortened from 6000-25000 ms to 150-300 ms.  Source
%%      hygiene only -- no LTS impact.
%%
%%   6. Banner / file inlining: GRAPH.RDR, MISC.RDR, SCREEN.RDR are
%%      inlined into a single self-contained file.  No LTS impact.
%%
...
%%   true.
%%
%%   %% New in 19880114: a station with no phone number can still be
%%   %% entered in Interactive mode (development convenience).
%%   ?- initial_state(S0),
...
%%   M = interactive.
%%
%%   %% F4 RadioTx is gone -- no such action is enabled.
