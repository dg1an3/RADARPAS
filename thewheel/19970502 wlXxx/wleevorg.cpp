...
wlEevorg::wlEevorg(wlEevorg *fromParent, int mutFlag) : wlObject(){	// copy constructor
		...
		if (mutFlag) {	// generate a single mutation
				...
				while (ruleVal > maxVal) {	// create three new rules
						rule.Add(0);
						rule.Add(0);
						rule.Add(0);
						maxVal++;
...
void wlEevorg::Serialize(CArchive& ar) {
		if (ar.IsStoring()) {
				ar << (WORD)maxVal;
				ar << (WORD)maxRule;
		} 
...
BOOL wlEevorg::Draw(CDC* pDC, LPCRECT lpBounds, int *nextScaleHint) {
