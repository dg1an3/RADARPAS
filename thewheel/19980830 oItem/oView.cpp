...
BOOL oView::PreCreateWindow(CREATESTRUCT& cs) {
...
void oView::OnSetFocus(CWnd* pOldWnd) {
...
void oView::OnSize(UINT nType, int cx, int cy) {
...
void oView::AssertValid() const {
	CView::AssertValid();
}

void oView::Dump(CDumpContext& dc) const {
	CView::Dump(dc);
}

oDocument *oView::GetDocument() { // non-debug version is inline
...
oItem *oView::HitTestItems(CPoint point) {
...
void oView::SetSelection(oItem *toItem) {
...
void oView::OnLButtonDown(UINT nFlags, CPoint point)  {
...
void oView::SetupTracker(oItem *forItem, CRectTracker *tracker) {
...
void oView::OnLButtonDblClk(UINT nFlags, CPoint point)  {
...
BOOL oView::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)  {
...
void oView::OnUpdateEditClear(CCmdUI* pCmdUI) {
	pCmdUI->Enable(mySelection != NULL);
}

void oView::OnEditCopy() {
	if (mySelection)
		mySelection->CopyToClipboard();
}

void oView::OnUpdateEditCopy(CCmdUI* pCmdUI) {
	pCmdUI->Enable(mySelection != NULL);
}

void oView::OnEditPaste() {
	oItem* item = NULL;
	TRY {
		// Create new item connected to this document.
		oDocument* myDoc = GetDocument();    
...
void oView::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint) {
...
void oView::OnSizingItems(DWORD time, CPoint mouse) {
	oDocument *myDoc = GetDocument();
	ASSERT_VALID(myDoc);
...
void oView::OnPosingItems(DWORD time) {
	// oDocument *myDoc = GetDocument();
	// ASSERT_VALID(myDoc);
...
void oView::OnMouseMove(UINT nFlags, CPoint point) {
...
void oView::dTotalActivation(double delta) {
	totalActivation += delta;
}

void oView::OnDropFiles(HDROP hDropInfo)  {
...
void  oView::drawLinks( CDC *pDC ) {
	oDocument *myDoc = GetDocument();
 	POSITION pos = myDoc->GetStartPosition();
	while (pos != NULL)	{
		oItem *anItem = (oItem *)myDoc->GetNextItem(pos);
		CPoint center = anItem->myRect.CenterPoint();
		POSITION pos2 = myDoc->GetStartPosition();
		while (pos2 != NULL) {
			oItem *anItem2 = (oItem *)myDoc->GetNextItem(pos2);
			if (anItem != anItem2) {
				CPoint center2 = anItem2->myRect.CenterPoint();
				pDC->MoveTo( center );
				pDC->LineTo( center2 );
			}
		}
